...
class CellProbabilities:
    ...
    def __init__(self, p_veg: float, p_den: float, p_wind: float, p_slope: float, p_h: float):
        self.probability_vector = np.array([p_veg, p_den, p_wind, p_slope, p_h]) 
    ...
    def from_vector(cls, probability_vector: np.ndarray):
        if probability_vector.shape != CellProbabilities.P_VECTOR_SIZE:
            raise Exception(f"Incorrect shape for vector parameter, try: {CellProbabilities.P_VECTOR_SIZE}")
        return CellProbabilities(probability_vector[0], probability_vector[1], 
                                 probability_vector[2], probability_vector[3], 
...
class Cell:
    ...
    @classmethod
    def from_values(cls, p_veg: float, p_den: float, p_wind: float, p_slope: float, p_h: float):
